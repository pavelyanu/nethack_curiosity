...
def alloc_trajectory_tensors(
    env_info: EnvInfo, num_traj, rollout, rnn_size, device, share
) -> TensorDict:
    ...
    for space_name, space in obs_space.spaces.items():
        tensors["obs"][space_name] = init_tensor(
            [num_traj, rollout + 1], space.dtype, space.shape, device, share
        )
    tensors["rnn_states"] = init_tensor(
        [num_traj, rollout + 1], torch.float32, [rnn_size], device, share
    )

    num_actions, num_action_distribution_parameters = action_info(env_info)
    policy_outputs = policy_output_shapes(
        num_actions, num_action_distribution_parameters
    )
    ...
    # MY CODE BLOCK #
    #################
    outputs_with_extra_rollout_step.append("intrinsic_values")
    #################

    for name, shape in policy_outputs:
        assert name not in tensors
        rollout_len = (
            rollout + 1 if name in outputs_with_extra_rollout_step else rollout
        )
        tensors[name] = init_tensor(
            [num_traj, rollout_len], torch.float32, shape, device, share
        )

    # env outputs
    tensors["rewards"] = init_tensor(
        [num_traj, rollout], torch.float32, [], device, share
    )
    tensors["rewards"].fill_(
        -42.42
    )  # if we're using uninitialized values by mistake it will be obvious
    tensors["dones"] = init_tensor([num_traj, rollout], torch.bool, [], device, share)
    tensors["dones"].fill_(True)
    tensors["time_outs"] = init_tensor(
        [num_traj, rollout], torch.bool, [], device, share
    )
    tensors["time_outs"].fill_(False)  # no timeouts by default
    tensors["policy_id"] = init_tensor(
        [num_traj, rollout], torch.int, [], device, share
    )
    tensors["policy_id"].fill_(
        -1
    )  # -1 is an invalid policy index, experience from policy "-1" is always ignored
    tensors["valids"] = init_tensor(
        [num_traj, rollout + 1], torch.bool, [], device, share
    )
...
def alloc_policy_output_tensors(cfg, env_info: EnvInfo, rnn_size, device, share):
    ...
    policy_outputs += [
        ("new_rnn_states", [rnn_size])
    ]  # different name so we don't override current step rnn_state
    ...
    if cfg.batched_sampling:
        ...
        for name, shape in policy_outputs:
            policy_output_tensors[name] = init_tensor(
                policy_outputs_shape, torch.float32, shape, device, share
            )
    else:
        ...
        policy_output_tensors = init_tensor(
            policy_outputs_shape, torch.float32, [outputs_combined_size], device, share
        )

    return policy_output_tensors, output_names, output_sizes


class IntrinsicRewardBufferMgr(Configurable):
    def __init__(self, cfg, env_info: EnvInfo):
        ...
        for i in range(cfg.num_workers):
            ...
            buffers_for_device = (
                self.buffers_per_device.get(sampling_device, 0) + num_buffers
            )
            self.buffers_per_device[sampling_device] = buffers_for_device

        rnn_size = get_rnn_size(cfg)  # in case we have RNNs

        self.trajectories_per_training_iteration = trajectories_per_training_iteration(
            cfg
        )

        if cfg.batched_sampling:
            worker_traj_per_iteration = (
                env_info.num_agents * cfg.num_envs_per_worker
        ...
        if not cfg.async_rl:
            log.debug(
                "In synchronous mode, we only accumulate one batch. Setting num_batches_to_accumulate to 1"
            )
        ...
        for device, num_buffers in self.buffers_per_device.items():
            ...
            num_buffers = max(
                num_buffers,
                self.max_batches_to_accumulate
            ...
            self.policy_output_tensors_torch[device], output_names, output_sizes = (
                alloc_policy_output_tensors(cfg, env_info, rnn_size, device, share)
            )
            ...
            if cfg.batched_sampling:
                ...
                for i in range(
                    0, num_buffers, self.sampling_trajectories_per_iteration
                ):
                    self.traj_buffer_queues[device].put(
                        slice(i, i + self.sampling_trajectories_per_iteration)
                    )
            else:
