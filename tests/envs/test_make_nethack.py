...
def test_make_nethack():
    ...
    env = make_nethack('NetHackScore', penalty_mode='exp', penalty_step=-0.02, penalty_time=0.1)
    ...
    with pytest.raises(ValueError):
        env = make_nethack('InvalidTaskName')


class TestMakeNetHack:
    def test_name_parameter(self):
        score_names = ['score', 'nethack_score', 'NetHackScore']
        staircase_pet_names = ['staircase_pet', 'nethack_staircase_pet', 'NetHackStaircasePet']
